...
import { useCreateActualTrade } from '@/hooks'

export default function TradeCreate() {
  const navigate = useNavigate()
  const createMutation = useCreateActualTrade()
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    setError('')
    if (!form.stockCode || !form.price || !form.quantity || !form.tradeDate) {
      setError('请填写必填项')
      return
    }
    try {
  ...
  return (
    <div className="p-6 space-y-4 max-w-xl">
      <div className="flex items-center gap-3">
        ...
        <h2 className="text-base font-medium text-gray-200">录入实盘</h2>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5 bg-gray-900 border border-gray-800 rounded-lg p-5">
        ...
        {form.direction === 'SELL' && (
          <div className="bg-amber-950 border border-amber-900 rounded p-3 text-sm text-amber-400">
            ⚠️ 卖出时系统将按 FIFO 规则匹配最早买入记录，盈亏由系统自动计算
          </div>
        )}
        ...
        <div className="flex justify-end gap-3 pt-2">
          ...
          >
            {createMutation.isPending ? '录入中…' : '录入'}
          </button>
        </div>
      </form>
    </div>
